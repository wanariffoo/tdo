...
bool Solver::precond(double* d_c, double* d_r)
{
    ...
    // // c.resize(d.size()); 
    // // c = 0.0;
	// setToZero<<<m_gridDim[m_topLev], m_blockDim[m_topLev]>>>(d_c, 18);

    // // // Vector<double> rtmp(r);
	// vectorEquals_GPU<<<m_gridDim[m_topLev], m_blockDim[m_topLev]>>>(m_d_rtmp[m_topLev], d_r, 18);

    
	// // NOTE: the original d_c and d_r from i_s.cu stay here
	// // d_gmg_c[topLev] = d_c
	// // d_gmg_r[topLev] = d_r
	// vectorEquals_GPU<<<m_gridDim[m_topLev], m_blockDim[m_topLev]>>>(m_d_gmg_c[m_topLev], d_c, 18);
	// cudaDeviceSynchronize();
	// vectorEquals_GPU<<<m_gridDim[m_topLev], m_blockDim[m_topLev]>>>(m_d_gmg_r[m_topLev], d_r, 18);
	// cudaDeviceSynchronize();
...
bool Solver::precond_add_update_GPU(double* d_c, double* d_r, std::size_t lev, int cycle)
{
    ...
    //      // c += ctmp;
    //     addVector_GPU<<<m_gridDim[lev], m_blockDim[lev]>>>( d_c, m_d_ctmp[lev], m_num_rows );

    //     UpdateResiduum_GPU<<< m_gridDim[lev], m_blockDim[lev] >>>(m_num_rows, m_max_row_size, m_d_value, m_d_index, m_d_ctmp[lev], d_r);

    // }

    // // restrict defect
    // setToZero<<<m_gridDim_cols[lev-1],m_blockDim_cols[lev-1]>>>( m_d_gmg_r[lev-1], 8 );
