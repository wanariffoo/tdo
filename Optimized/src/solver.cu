...
bool Solver::base_solve(double* d_bs_u, double* d_bs_b, ofstream& ofssbm)
{
    ...
    ComputeResiduum_GPU_<<<m_gridDim[0],m_blockDim[0]>>>(m_num_rows[0], m_max_row_size[0], m_d_value[0], m_d_index[0], d_bs_u, m_d_bs_r, d_bs_b);
    ...
    else
    {
        ...
        while(m_bs_foo || bs_step < m_bs_maxIter)
        {
            // precond, z = r B
                cudaEventRecord(start);
            Jacobi_Precond_GPU_<<<m_gridDim[0], m_blockDim[0]>>>(m_d_bs_z, m_d_value[0], m_d_index[0], m_max_row_size[0], m_d_bs_r, m_num_rows[0], m_damp);
                cudaEventRecord(stop);
                cudaEventSynchronize(stop);
                milliseconds = 0;
                cudaEventElapsedTime(&milliseconds, start, stop);
            ...
            /// z = A*p
                cudaEventRecord(start);
            Apply_GPU_<<<m_gridDim[0],m_blockDim[0]>>>( m_num_rows[0], m_max_row_size[0], m_d_value[0], m_d_index[0], m_d_bs_p, m_d_bs_z );
                cudaEventRecord(stop);
                cudaEventSynchronize(stop);
                milliseconds = 0;
                cudaEventElapsedTime(&milliseconds, start, stop);
...
bool Solver::precond_add_update_GPU(double* d_c, double* d_r, std::size_t lev, int cycle, ofstream& ofssbm)
{
    ...
    // ctmp = P[lev-1] * c_coarse;
            cudaEventRecord(start);
    Apply_GPU_<<<m_gridDim[lev],m_blockDim[lev]>>>( m_num_rows[lev], m_p_max_row_size[lev-1], m_d_p_value[lev-1], m_d_p_index[lev-1], m_d_gmg_c[lev-1], m_d_ctmp[lev]);
            cudaEventRecord(stop);
            cudaEventSynchronize(stop);
            milliseconds = 0;
            cudaEventElapsedTime(&milliseconds, start, stop);
...
bool Solver::smoother(double* d_c, double* d_r, int lev)
{
    ...
    Jacobi_Precond_GPU_<<<m_gridDim[lev], m_blockDim[lev]>>>(d_c, m_d_value[lev], m_d_index[lev], m_max_row_size[lev], d_r, m_num_rows[lev], m_damp);
...
bool Solver::solve(double* d_u, double* d_b, vector<double*> d_value, ofstream& ofssbm)
{
    ...
    while(m_foo)
    {
        ...
        dotProduct(m_d_rho, m_d_r, m_d_z, m_num_rows[m_topLev], m_gridDim[m_topLev], m_blockDim[m_topLev]);
            ...
            milliseconds = 0;
            cudaEventElapsedTime(&milliseconds, start, stop);
            // if ( base_counter == 0 ) ofssbm << "   dotProduct()\t\t" << milliseconds << endl;
        ...
        /// z = A*p
            cudaEventRecord(start);
        Apply_GPU_<<<m_gridDim[m_topLev],m_blockDim[m_topLev]>>>( m_num_rows[m_topLev], m_max_row_size[m_topLev], m_d_value[m_topLev], m_d_index[m_topLev], m_d_p, m_d_z );
            cudaEventRecord(stop);
            cudaEventSynchronize(stop);
            milliseconds = 0;
            cudaEventElapsedTime(&milliseconds, start, stop);
        ...
        //NOTE: potential cause of segmentation dump
        // alpha = rho / (p * z)
            cudaEventRecord(start);
        ...
        // print_GPU<<<1,1>>>( m_d_alpha );
        // printVector_GPU<<<1,25>>>( m_d_r, 25 );
        ...
        // if ( m_counter_it == 0 )
        // {
        //     printVector(m_d_p, 81);
        //     // print_GPU<<<1,1>>>( m_d_rho );
        //     // printVector_GPU<<<1,80>>>( m_d_z, 80 );
        // }
        ...
        // checkIterationConditionsBS<<<1,1>>>(m_d_foo, m_d_step, m_maxIter, m_d_res, m_d_minRes);
    ...
    //         cudaEventRecord(start);
    //     UpdateResiduum_GPU_<<<m_gridDim[m_topLev], m_blockDim[m_topLev]>>>( m_num_rows[m_topLev], m_max_row_size[m_topLev], m_d_value[m_topLev], m_d_index[m_topLev], m_d_c, m_d_r );
    //         cudaEventRecord(stop);
    //         cudaEventSynchronize(stop);
    //         milliseconds = 0;
    //         cudaEventElapsedTime(&milliseconds, start, stop);
    ...
    // if ( bm_switch == 0 ) ofssbm << "Number of steps \t\t" << m_counter_it << endl;
    // if ( bm_switch == 0 ) ofssbm << "Average time per iteration step " << m_sum_it/m_counter_it << endl;
